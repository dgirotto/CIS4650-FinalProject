...
void visit(TreeNode* theNode){

	if(theNode->nodekind==StmtK){
		switch(theNode->kind.stmt){
			// scopeDepth++ for IfK, WhileK, CmpdK ? 
			case IfK:
				break;
			case WhileK:
				break;
			...
			case FunK:
				...
				//scopeDepth++; // is this valid? 
				fprintf(listing,"Fun %s\n",theNode->attr.name);
				break;
			default:
				fprintf(listing,"Invalid Dec\n");		 
                }
	}
	else{
