...
var_declaration		: type_specifier getID SEMI
			...
			| type_specifier getID LSQ NUM RSQ SEMI
			{
			   $$ = newDecNode(VarK);
                           $$->child[0] = $1;
			   $$->child[1] = newExpNode(ConstK);
			   $$->child[1]->attr.val = 99999; //test
                           $$->attr.name = savedName; //store the ID
			}
			;
...
fun_declaration		: type_specifier getID LPAREN params RPAREN compound_stmt
			{
			   $$ = newDecNode(FunK);
			   $$->child[0] = $1;
			   $$->child[1] = $4;
