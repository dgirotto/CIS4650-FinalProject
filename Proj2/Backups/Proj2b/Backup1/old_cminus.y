...
var_declaration		: type_specifier ID 
			{
			   savedVarName = copyString(tokenString); //store the ID
			   printf("savedVarName is %s\n",savedVarName);
			   savedLineNo = lineno;
			}
			SEMI
			{
			   $$ = newDecNode(VarK);
                           $$->child[0] = $1;
                           $$->attr.name = savedVarName; //store the ID
			   $$->lineno = savedLineNo;
			}
			| type_specifier ID
			{
...
fun_declaration		: type_specifier ID 
			{
			   savedFunName = copyString(tokenString);
			   savedLineNo = lineno;
			}
