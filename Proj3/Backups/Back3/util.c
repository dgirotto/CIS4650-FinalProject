...
//static int varType; // 0 = void, 1 = int, 2 = void [], 3 = int []
static int semanticErr=0;
static Entry * temp;
static char * currentFun = NULL;
static int returnType; // function return type: 0 = void, 1 = int
static int idType; // for type checking 
static int tempType; // for operand type verification purposes 
			...
			case StmtK:
				switch(theNode->kind.stmt){
					...
					case ReturnK:
						...
						else{ /* returns void (return;) */
							if(temp->theTree->type != 0){
								fprintf(listing,"ERROR in return type\n");
							}
							//printf("In ReturnK... returning void\n");	
						}
...
printf(" **** ENTERING OPK **** \n");
						...
						postorder(theNode->child[0]);
						tempType = idType;
						printf("idType after ch1: %d\n",idType);
						postorder(theNode->child[1]);
						printf("idType after ch2: %d\n",idType);
						if(tempType != idType){
...
printf("The Id's type is %d\n",temp->theTree->type);
printf("The Id's name is %s\n",temp->name);
			...
			case DecK:
				switch(theNode->kind.dec){
					...
					case FunK:
						...
						theNode->type = theNode->child[0]->attr.val;
printf("The function's type is %d\n",theNode->type);
