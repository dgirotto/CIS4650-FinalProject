			...
			case StmtK:
				switch(theNode->kind.stmt){
					case IfK:
						...
						if(theNode->child[0]->type != 1){
							fprintf(listing,"ERROR on line %d! Invalid IF test expression.\n",theNode->lineno);
						}
					...
					case WhileK:
						...
						if(theNode->child[0]->type != 1){
                                                        fprintf(listing,"ERROR on line %d! Invalid WHILE test expression.\n",theNode->lineno);
                                                }
					...
					case ReturnK:
						...
						if(theNode->child[0] != NULL){
							...
							postorder(theNode->child[0]);
							temp = lookup(currentFun,0,1);

							if(theNode->child[0]->type != temp->theTree->type){
								semanticErr++;
								fprintf(listing,"ERROR on line %d! Return type is of incorrect type\n",theNode->lineno);							
							}
							else{
					...
					case AssignK:
						...
						if(theNode->child[0]->type != theNode->child[1]->type){	
							semanticErr++;
							fprintf(listing,"ERROR on line %d! Assignment made with incorrect type.\n",theNode->lineno);
						}
						else{
							theNode->type = theNode->child[0]->type;
						}
					...
					default:
						fprintf(listing,"Invalid Stmt kind!\n");
						break;
				}
				break;
			case ExpK:
				switch(theNode->kind.exp){
					case OpK:
						...
						if(theNode->child[0]->type != theNode->child[1]->type){
							semanticErr++;
							fprintf(listing,"ERROR on line %d! Operand types are not the same.\n",theNode->lineno);
						}
						else{
							theNode->type = theNode->child[0]->type;
						}	
					...
					case IdK:
						...
						if(temp == NULL){
							semanticErr++;
							fprintf(listing,"ERROR on line %d! '%s' not declared/defined!\n",theNode->lineno,theNode->attr.name);
						}
					...
					case CallK:
						if((strcmp(theNode->attr.name,"input")!=0) && (strcmp(theNode->attr.name,"output")!=0)){
							...
							if(temp == NULL){
								semanticErr++;
								fprintf(listing,"ERROR on line %d! '%s' does not exist! Must declare before using.\n",theNode->lineno,theNode->attr.name);
							}
							else{
								// determine if the variable is accessible from current depth level
								if(temp->depth > scopeDepth){
									semanticErr++;
									fprintf(listing,"ERROR on line %d! '%s' is not accessible from current scope!\n",theNode->lineno,theNode->attr.name);
								}
							}
						}
						postorder(theNode->child[0]);
					...
					case SubsK:
						...
						if(temp == NULL){
							...
							theNode->type = temp->theTree->type;
							postorder(theNode->child[0]);
							temp = lookup(theNode->attr.name,scopeDepth,1);
							if(theNode->child[0]->attr.val < 0 || theNode->child[0]->attr.val >= temp->theTree->size){
								semanticErr++;
								fprintf(listing,"ERROR on line %d! Index of %d is out of array bounds.\n",theNode->lineno,theNode->child[0]->attr.val);
							}
						}
						break;
						
					default:
						fprintf(listing,"Invalid Exp kind!\n");
						break;
				}
				break;
			case DecK:
				switch(theNode->kind.dec){
					case VarK:
						...
						if(lookup(theNode->attr.name,scopeDepth,0) == NULL){
							...
							fprintf(listing,"Adding Var: %s to Hash Table. Scope Depth: %d. Type: %d\n",theNode->attr.name,scopeDepth,theNode->type);
						}
						else{
							semanticErr++;
							fprintf(listing,"ERROR on line %d! Variable '%s' is already defined!\n",theNode->lineno,theNode->attr.name);
						}
						break;
					...
					case FunK:
						...
						if(lookup(theNode->attr.name,scopeDepth,0) == NULL){
							insert(theNode->attr.name,theNode,scopeDepth);
							fprintf(listing,"Adding Function: %s to Hash Table. Scope Depth: %d. Type: %d\n",theNode->attr.name,scopeDepth,theNode->type);

						}
						else{
							semanticErr++;
							fprintf(listing,"ERROR on line %d! Function '%s' is already defined!\n",theNode->lineno,theNode->attr.name);
						}
					...
					case ParamK:
						...
						if(lookup(theNode->attr.name,scopeDepth,0) == NULL){
							...
							insert(theNode->attr.name,theNode,scopeDepth);
							fprintf(listing,"Adding Param: %s to Hash Table. Scope Depth: %d. Type: %d\n",theNode->attr.name,scopeDepth,theNode->type);

						}
						else{
							semanticErr++;
							fprintf(listing,"ERROR on line %d! Parameter '%s' is already defined!\n",theNode->lineno,theNode->attr.name);
						}
				...
				break;
			default:
				fprintf(listing,"Invalid node kind: %d \n",theNode->nodekind);
				break;
		}
...
void deleteScope(int depth){
	...
	for(i=0; i<MAXSIZE; i++){
		...
		if(iter!=NULL){
			...
			if(iter->depth == depth){
				fprintf(listing,"Removing %s from the symbol table of depth %d.\n",iter->name,depth);
